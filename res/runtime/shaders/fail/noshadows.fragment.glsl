...
in vec3 gs_position_modelspace;
in vec2 gs_texcoord_modelspace;
in vec3 gs_normal_modelspace;

in mat4 gs_modelMatrix;
in mat4 gs_viewMatrix;
in mat3 gs_tbnMatrix;
...
const float lightWattage = 500;

const vec3 position_worldspace = (gs_modelMatrix * vec4(gs_position_modelspace, 1.0)).xyz;
const vec3 position_cameraspace = (gs_viewMatrix * vec4(position_worldspace, 1.0)).xyz;

const vec3 cameraPosition_cameraspace = vec3(0, 0, 0);
const vec3 eyeDirection_cameraspace = vec3(0, 0, 0) - position_cameraspace;
const vec3 eyeDirection_tangentspace = gs_tbnMatrix * eyeDirection_cameraspace;
...
const vec3 lightDirection_tangentspace = gs_tbnMatrix * lightDirection_cameraspace;

vec2 getTexcoordOffset()
{
	return gs_texcoord_modelspace + lightDirection_tangentspace.xy * (texture2D(parallaxMapSampler, gs_texcoord_modelspace).r * parallaxMultiplier + parallaxBias);
}
...
{
	vec3 lightPos_cameraspace = (gs_viewMatrix * vec4(l.pos, 1.0)).xyz;
	vec3 displacementFromLight = lightPos_cameraspace - position_cameraspace;
	float cosTheta = clamp(dot(parsedNormal_tangentspace, gs_tbnMatrix * displacementFromLight), 0.0, 1.0);
...
vec4 getSpecularComponentFromLight(BaseLight l, vec3 parsedNormal_tangentspace)
{
	...
	vec3 displacementFromLight = lightPos_cameraspace - position_cameraspace;
	vec3 E = normalize(eyeDirection_tangentspace);
	vec3 R = reflect(-normalize(gs_tbnMatrix * displacementFromLight), parsedNormal_tangentspace);
	float cosAlpha = clamp(dot(E, R), 0, 1);
	float distanceFromLight = length(displacementFromLight);
