...
public:
	...
	std::vector<Object>& get_objects_r();
	std::vector<Entity>& get_entities_r();
	std::vector<EntityObject>& get_entity_objects_r();
	const Vector3F& get_gravity() const;
	const Vector3F& get_spawn_point() const;
	const Vector3F& get_spawn_orientation() const;
	Vector3F& get_gravity_r();
	Vector3F& get_spawn_point_r();
	Vector3F& get_spawn_orientation_r();
	const std::string& get_world_link() const;
	const std::map<std::array<GLint, tz::graphics::light_number_of_uniforms>, Light>& get_lights() const;
	std::map<std::array<GLint, tz::graphics::light_number_of_uniforms>, Light>& get_lights_r();
private:
