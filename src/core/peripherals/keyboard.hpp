...
namespace tz
{
	...
	struct KeyInfo
	{
		...
		bool operator==(const KeyInfo& key) const {return this->code == key.code;}
	};

	namespace peripherals::keyboard
	{
		/**
		 * @ingroup tz_core_peripherals_keyboard
		...
		constexpr KeyInfo get_key(KeyCode code)
		{
			...
			{
				if(detail::key_codes[i].code == code)
				{
					return detail::key_codes[i];
				}
			}
			return key_null;
		}
		...
		constexpr KeyInfo get_key(char representation)
		{
			...
			for(std::size_t i = 0; i < static_cast<int>(KeyCode::Count); i++)
			{
				if(static_tolower_dirty(detail::key_codes[i].representation) == representation)
				{
					return detail::key_codes[i];
				}
			}
