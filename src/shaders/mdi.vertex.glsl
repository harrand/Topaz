...
layout(std430, row_major, binding = 0) buffer MVPSSBO
{
    mat4 ssbo_model;
    mat4 ssbo_view;
    mat4 ssbo_projection;
};
...
void share()
{
	...
	projection_matrix = p;
	*/
	model_matrix = ssbo_model;
	view_matrix = ssbo_view;
	projection_matrix = ssbo_projection;
...
{
	share();
	gl_Position = ssbo_projection * ssbo_view * ssbo_model * vec4(position_modelspace, 1.0);
	position_lightspace = light_viewprojection * ssbo_model * vec4(position_modelspace, 1.0);
	/*
	if(is_instanced)
	{
