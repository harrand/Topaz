...
namespace tz::gl
{
	...
	depth_attachment(info.depth != nullptr ? static_cast<image_component*>(info.depth) : nullptr)
	{
		...
		}), "ImageOutput contains colour attachments, but one or more of them is missing resource_flag::renderer_output. An image resource cannot be an image output attachment unless it contains this flag.");

		if(this->depth_attachment != nullptr)
		{
			hdk::assert(info.depth->get_resource() != nullptr && info.depth->get_resource()->get_type() == resource_type::image, "Provided an icomponent to an ImageOutput depth attachment which was not an ImageResource. Please submit a bug report.");
		}
	...
	const image_component& ImageOutput::get_colour_attachment(std::size_t colour_attachment_idx) const
	{
		return *this->colour_attachments[colour_attachment_idx];
	}

	image_component& ImageOutput::get_colour_attachment(std::size_t colour_attachment_idx)
	{
		return *this->colour_attachments[colour_attachment_idx];
	}

	const image_component& ImageOutput::get_depth_attachment() const
	{
	...
	image_component& ImageOutput::get_depth_attachment()
	{
		hdk::assert(this->has_depth_attachment(), "Requested ImageOutput depth attachment, but there isn't one. Please submit a bug report.");
		return *this->depth_attachment;
	}
