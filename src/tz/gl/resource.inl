...
namespace tz::gl
{
	...
	BufferResource BufferResource::from_many(R&& data, BufferInfo info)
	{
		...
		hdk::assert(success, "Storage for a single T (sizeof T + alignof T) was too small! Please submit a bug report.");

		std::memcpy(data_ptr, std::ranges::data(data), sizeof(T) * data.size());
		hdk::assert(space_copy >= space, "Alignment of pointer somehow increased space, this is a logic error. Please submit a bug report.");
