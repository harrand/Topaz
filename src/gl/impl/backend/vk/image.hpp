...
namespace tz::gl::vk
{
	...
	public:
		...
		Image(const LogicalDevice& device, VkImage swapchain_image_native, std::uint32_t width, std::uint32_t height, Format format, UsageField usage);
	...
	protected:
		VkImage image;
		std::optional<VmaAllocation> alloc;
