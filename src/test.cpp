...
void init()
{
	...
	std::vector<Object> instancified_world = tz::graphics::instancify_full(engine.world.get_objects());
	for(Object& object : instancified_world)
		object.position += Vector3F(0, 750, 0);
	
	TrivialFunctor render_instancified([&](){for(Object& object : instancified_world) object.render(engine.camera, &(engine.default_shader), wnd.get_width(), wnd.get_height());});
	engine.add_update_command(&render_instancified);
