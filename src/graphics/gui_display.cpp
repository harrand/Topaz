...
void Label::render(Shader& shader, int window_width_pixels, int window_height_pixels) const
{
    ...
    Matrix4x4 model = projection * tz::transform::model(Vector3F(this->get_x(), this->get_y() + this->get_height(), 0.0f), Vector3F(0, consts::pi, consts::pi), Vector3F(this->get_width(), this->get_height(), 0.0f));
    shader.set_uniform<Matrix4x4>("model_matrix", model);
