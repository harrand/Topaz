...
void Shader::add_uniform(Uniform<T>&& uniform)
{
	...
	{
		tz::debug::print("[Shader]: Tried to add uniform with location '", uniform.get_uniform_location(), "' to Shader with handle ", this->program_handle, ", of which the location is already occupied.\n");
		return;
	}
	if(this->uniform_counter >= tz::graphics::maximum_uniforms)
	{
		tz::debug::print("[Shader]: Tried to add uniform with location '", uniform.get_uniform_location(), "' to Shader with handle ", this->program_handle, ", but it already has the maximum number of uniforms attached (", tz::graphics::maximum_uniforms, ").\n");
		return;
	}
...
T Shader::get_uniform_value(std::string_view uniform_location) const
{
	if(!this->has_uniform(uniform_location))
	{
		tz::debug::print("[Shader]: Tried to retrieve uniform_value for non-existent uniform. Returning default...\n");
		return T();
	}
