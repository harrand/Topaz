...
Texture::Texture(Bitmap<Pixel> pixel_data): Texture(pixel_data.width, pixel_data.height, false, tz::graphics::TextureComponent::COLOUR_TEXTURE)
{
	...
	switch(this->texture_component)
	{
		...
		case tz::graphics::TextureComponent::COLOUR_TEXTURE:
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, this->width, this->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image_data.data());
			break;
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, this->width, this->height, 0, GL_RGBA8, GL_UNSIGNED_BYTE, image_data.data());
	 */
	// Unbind the texture.
	glBindTexture(GL_TEXTURE_2D, 0);
}
