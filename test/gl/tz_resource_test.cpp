...
void null_buffer()
{
	hdk::assert(tz::gl::buffer_resource::null().is_null(), "Null buffer is not a null buffer");
	hdk::assert(tz::gl::buffer_resource::null().get_type() == tz::gl::resource_type::buffer, "Null buffer is not a buffer at all!");
}

//--------------------------------------------------------------------------------------------------
void null_image()
{
	hdk::assert(tz::gl::image_resource::null().is_null(), "Null image is not a null image");
	hdk::assert(tz::gl::image_resource::null().get_type() == tz::gl::resource_type::image, "Null image is not an image at all!");
}
...
void basic_buffer()
{
	// Check that buffer_resource::from_one returns expected data size.
	{
		...
		hdk::assert(!buf.is_null(), "buffer_resource::from_one(5u) is wrongly considered a null buffer.");
		hdk::assert(buf.get_type() == tz::gl::resource_type::buffer, "buffer_resource::from_one(5u) is not resource_type::buffer!");
		auto span = buf.data_as<const unsigned int>();
		hdk::assert(span.size() == 1, "buffer_resource::from_one(5u) -> data_as<uint>.size() == %zu when it should be %d", span.size(), 1);
	}
	// Check that buffer_resource::from_many returns expected data size.
	{
		auto buf = tz::gl::buffer_resource::from_many({1.0f, 2.0f, 3.0f});
		hdk::assert(buf.get_type() == tz::gl::resource_type::buffer, "buffer_resource::from_many({1.0f, 2.0f, 3.0f}) is not resource_type::buffer!");
		hdk::assert(!buf.is_null(), "buffer_resource::from_many({1.0f, 2.0f, 3.0f}) is wrongly considered a null buffer.");
		auto span = buf.data_as<const float>();
		std::array<float, 3> expected{1.0f, 2.0f, 3.0f};
		hdk::assert(std::equal(span.begin(), span.end(), expected.begin()), "buffer_resource::from_many({1.0f, 2.0f, 3.0f}) != array<float, 3>{1.0f, 2.0f, 3.0f}");
	}
}
...
{
	{
		...
		hdk::assert(!img.is_null(), "image_resource::from_unitialised(...) is wrongly considered a null image resource.");
		hdk::assert(img.get_type() == tz::gl::resource_type::image, "image_resource::from_uninitialised(...) is not resource_type::image!");
		hdk::assert(img.get_dimensions() == dims, "image_resource::from_uninitialised{..., .dimensions = {%uu, %uu}}.get_dimensions() wrongly == {%u, %u}", dims[0], dims[1], img.get_dimensions()[0], img.get_dimensions()[1]);
