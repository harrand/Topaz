...
void null_buffer()
{
	...
	hdk::assert(tz::gl::BufferResource::null().get_type() == tz::gl::ResourceType::Buffer, "Null buffer is not a buffer at all!");
}
...
void null_image()
{
	...
	hdk::assert(tz::gl::ImageResource::null().get_type() == tz::gl::ResourceType::Image, "Null image is not an image at all!");
}
...
void basic_buffer()
{
	// Check that BufferResource::from_one returns expected data size.
	{
		...
		hdk::assert(buf.get_type() == tz::gl::ResourceType::Buffer, "BufferResource::from_one(5u) is not ResourceType::Buffer!");
	...
	// Check that BufferResource::from_many returns expected data size.
	{
		...
		hdk::assert(buf.get_type() == tz::gl::ResourceType::Buffer, "BufferResource::from_many({1.0f, 2.0f, 3.0f}) is not ResourceType::Buffer!");
...
{
	{
		...
		hdk::assert(img.get_type() == tz::gl::ResourceType::Image, "ImageResource::from_uninitialised(...) is not ResourceType::Image!");
...
void default_arguments_buffer()
{
	...
	hdk::assert(nullbuf.get_access() == tz::gl::ResourceAccess::StaticFixed, "Null Buffer must be ResourceAccess::StaticFixed, but it is not.");
	hdk::assert(nullbuf.get_flags().empty(), "Null Buffer must not have any flags");

	{
		auto buf = tz::gl::BufferResource::from_one(1);
		hdk::assert(buf.get_access() == tz::gl::ResourceAccess::StaticFixed, "Defaulted Buffer must be ResourceAccess::StaticFixed, but it is not.");
...
void default_arguments_image()
{
	...
	hdk::assert(nullimg.get_access() == tz::gl::ResourceAccess::StaticFixed, "Null Image must be ResourceAccess::StaticFixed, but it is not.");
